/* app.js (FINAL) — THBC → PHL Package Auto-Stake
...
// ✅ ตามที่คุณยืนยัน: pool มีแล้ว (poolCount=1) และใช้ poolId 0
const POOL_ID = 0;
// ✅ ในโปรเจกต์นี้ใช้ package 0 (แก้เลขได้ถ้าคุณตั้งคนละ id)
const PACKAGE_ID = 0;
...
let stakeIndex = null;      // selected stake index
...
async function loadPoolAndPackage() {
  // 1) verify pool exists
  const pc = Number(await contractRO.poolCount());
  if (pc <= POOL_ID) throw new Error(`BAD_POOL: poolCount=${pc}, poolId=${POOL_ID}`);

  const [outToken, apyBP, lockSec, enabled, packageCount] = await contractRO.getPool(POOL_ID);

  if (!enabled) throw new Error("Pool is disabled");
  if ((outToken?.toLowerCase?.()||"") !== PHL_ADDRESS.toLowerCase()) throw new Error("Pool outToken is not PHL");

  // 2) verify package exists
  if (Number(packageCount) <= PACKAGE_ID) throw new Error(`BAD_PACKAGE: packageCount=${packageCount}, packageId=${PACKAGE_ID}`);

  const [thbcIn, principalOut, active] = await contractRO.getPackage(POOL_ID, PACKAGE_ID);
  if (!active) throw new Error("Package is not active");

  pkg = { thbcIn, principalOut };

  // update UI package line from on-chain data
  $("pkgText").textContent =
    `${ethers.formatUnits(thbcIn, thbcDec)} ${thbcSym} = ${ethers.formatUnits(principalOut, phlDec)} ${phlSym} (Buy → Auto-Stake)`;
  $("pkgHint").textContent =
    `Pool #${POOL_ID} • Package #${PACKAGE_ID} • APY ${(Number(apyBP)/100).toFixed(2)}% • Lock ${Math.round(Number(lockSec)/86400)} days`;
...
async function buyPackage() {
  ...
  // optional: check THBC balance for nicer error
...
$("btnBuy").addEventListener("click", async () => {
  try {
    ...
    // re-enable only if package loaded ok
    try { await loadPoolAndPackage(); } catch {}
  }
});
...
// if wallet changes, reload (simplest & most stable on mobile wallets)
if (window.ethereum) {
