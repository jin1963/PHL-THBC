...
// ✅ มีพูลเดียว => ใช้ poolId = 1 ตาม on-chain ที่คุณเช็คใน Remix
const POOL_ID = 1;
...
async function ensureBSC(injected) {
  ...
  try {
    ...
    // บาง Bitget ไม่ให้ switch จาก dapp => ให้ user สลับเอง
    return false;
  }
}
...
async function assertPoolOk() {
  ...
  if (pc <= POOL_ID) {
    throw new Error(`BAD_POOL: poolCount=${pc} แต่ DApp ใช้ poolId=${POOL_ID}`);
  }
  const p = await contractRO.getPool(POOL_ID);
  $("poolIdText").textContent = String(POOL_ID);

  if (!p.enabled) {
    throw new Error("Pool ยังไม่ enabled (ต้องเปิดใน owner)");
  }
  // outToken ควรเป็น PHL address (อ้างอิงได้ แต่ไม่บังคับ)
  return p;
}
...
function applyPackageView(p) {
  ...
  $("pkgMeta").textContent = `active: true`;
}

async function loadPackageFromChain(id) {
...
async function initPackageDropdown() {
  ...
  sel.onchange = async () => {
    ...
    try {
      ...
      const p = await loadPackageFromChain(id);
      applyPackageView(p);
      await refreshAllowanceAndButtons();
  ...
  $("pkgText").textContent = "-";
  $("pkgHint").textContent = "ยังไม่มี package active (ให้ owner ตั้ง setPackage)";
  $("pkgMeta").textContent = "-";
  setMsg("ไม่พบแพ็คเกจ active ในช่วง 0-9", "bad");
}

async function refreshBalances() {
  const [bThbc, bPhl] = await Promise.all([
    thbcRO.balanceOf(user),
    phlRO.balanceOf(user),
  ]);
  $("balThbcText").textContent = `${ethers.formatUnits(bThbc, thbcDec)} ${thbcSym}`;
  $("balPhlText").textContent = `${ethers.formatUnits(bPhl, phlDec)} ${phlSym}`;
}

async function refreshAllowanceAndButtons() {
  if (!selectedPackage) {
  ...
  $("allowText").textContent =
    `${ethers.formatUnits(allowance, thbcDec)} ${thbcSym}`;

  const need = selectedPackage.thbcIn;
  const okAllow = allowance >= need;

  // ปุ่ม approve ใช้เมื่อ allowance ไม่พอ
  $("btnApprove").disabled = okAllow;
  // buy ใช้ได้เมื่อ allowance พอ
  $("btnBuy").disabled = !okAllow;
}
...
async function loadStakeDetail() {
  ...
  $("stakeMeta").textContent =
    `Pool #${POOL_ID} • Index #${stakeIndex} • reward: ${ethers.formatUnits(reward, phlDec)} ${phlSym} • claimed: ${claimed ? "YES" : "NO"} • canClaim: ${can ? "YES" : "NO"}`;
...
async function connect() {
  try {
    ...
    // บาง Bitget ไม่ให้ switch จาก dapp => แสดงสถานะให้ user สลับเอง
    const switched = await ensureBSC(injected);
    if (!switched) {
      setMsg("กรุณาเลือกเครือข่าย BSC Mainnet (56) ในกระเป๋า แล้วกด Connect อีกครั้ง", "bad");
      return;
    }
...
$("btnApprove")?.addEventListener("click", async () => {
  try {
    ...
    await refreshAllowanceAndButtons().catch(()=>{});
  }
});
