...
async function ensureBSC(injected) {
  // บาง Bitget ไม่ชอบ switch ให้จับ error แล้วไปต่อ
  try {
...
async function detectPoolId() {
  ...
  if (pc <= 0) throw new Error("poolCount=0");
  for (let i = pc - 1; i >= 0; i--) {
    try {
      const p = await contractRO.getPool(i);
      if (p.enabled === true) return i;
    } catch {}
  }
  throw new Error("No enabled pool found");
}
...
async function updateAllowanceUI() {
  ...
  if (!user || POOL_ID === null) {
    meta.textContent = "Allowance: - • PoolId: -";
    return;
  }
  ...
  try {
    ...
    meta.textContent =
      `Allowance: ${ethers.formatUnits(allowance, thbcDec)} ${thbcSym} • PoolId: ${POOL_ID}`;
  } catch {
    meta.textContent = `Allowance: - • PoolId: ${POOL_ID ?? "-"}`;
  }
}
...
async function initPackageDropdown() {
  ...
  if (first) {
    ...
    resetPackageView();
    $("pkgHint").textContent = "ยังไม่มี package active (ต้องตั้งใน Owner/Remix: setPackage)";
    await updateAllowanceUI();
...
async function refreshAll() {
  ...
  if (POOL_ID === null) {
    POOL_ID = await detectPoolId(); // ✅ จะได้ 1 ตามรูปของคุณ
  }
...
async function buySelected() {
  ...
  if (allowance < selectedPackage.thbcIn) {
    throw new Error(`Allowance ไม่พอ กรุณากด Approve ก่อน`);
  }
...
async function connect() {
  try {
    ...
    POOL_ID = null; // ให้ตรวจใหม่เสมอหลัง connect
    clearStakeUI();
    resetPackageView();
...
$("btnApprove")?.addEventListener("click", async () => {
  try {
    ...
    $("btnApprove").disabled = !selectedPackage;
  }
});
