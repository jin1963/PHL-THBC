...
// ✅ สำคัญ: poolCount=1 => poolId ที่ถูกต้องมีแค่ 0
const POOL_ID = 0;

// dropdown packages 0..9
const PACKAGE_SCAN_MAX = 10;

// UI คงที่ตามที่คุณต้องการ
...
/* =========================
   Packages
========================= */
function setPoolStaticUI() {
  // หน้าเว็บแสดงสากลคงที่ตาม requirement
  // (ไม่พึ่ง getPool เพื่อกันบาง wallet revert)
  // Term/APY อยู่ใน HTML แล้ว หากอยากอัปเดตก็ทำได้ แต่ไม่จำเป็น
}
...
async function initPackageDropdown() {
  ...
  // ใช้ option ใน HTML ที่มีอยู่แล้ว (0-9)
  // แค่เปิดใช้งานหลัง connect
  sel.value = ""; // ยังไม่เลือก
  ...
  // auto: ลองหาแพ็คเกจแรกที่ active ใน 0-9
  ...
  if (first) {
    ...
    $("pkgHint").textContent = "ยังไม่มี package active (ต้องเปิดใน Remix ด้วย setPackage)";
  }
}
...
async function loadStakeList() {
  ...
  // ใส่รายการ stake ล่าสุดขึ้นก่อน
  const idx = [];
...
async function connect() {
  if (!window.ethereum) {
    setMsg("ไม่พบ Wallet (MetaMask/Bitget) กรุณาเปิดผ่าน Browser ของ Wallet", "bad");
    return;
  }
