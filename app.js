(() => {
  ...
  async function loadPool() {
    try {
      ...
      setText("poolApy", String(p.apyBP));
      setText("poolLock", String(p.lockSec));
      setText("poolEnabled", String(p.enabled));

      // note: packageCount_ อาจโชว์ 0 บางครั้ง (ไม่เป็นไร)
    } catch (e) {
  ...
  async function approveTHBC() {
    try {
      ...
      // approve อย่างน้อยเท่ากับ thbcIn (หรือจะ Max ก็ได้)
  ...
  async function loadStakes() {
    try {
      ...
      if (count === 0) {
        tbody.innerHTML = `<tr><td colspan="8" class="muted">ยังไม่มี stake</td></tr>`;
        return;
      }

      // load stakes sequential (ปลอดภัยกับ RPC)
      let rows = "";
      for (let i = 0; i < count; i++) {
        ...
        rows += `
          <tr>
            ...
            <td class="${claimed ? "ok" : ""}">${claimed ? "yes" : "no"}</td>
            <td class="${can ? "ok" : "muted"}">${can ? "yes" : "no"}</td>
            <td>
